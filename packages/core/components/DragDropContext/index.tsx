...
import { useDragHandleListener } from "../../lib/dnd/drag-handle";
...
const DragDropContextClient = ({
  ...
  const { getIsDraggingFromHandle } = useDragHandleListener();
    ...
    >
      <DragDropProvider
        ...
        onBeforeDragStart={(event) => {
          ...
          // The only point at which dnd-kit can still tell us the drag came
          // from a handle, so record it for the rest of the operation
          zoneStore.setState({
            draggedItem: event.operation.source,
            draggingFromHandle: getIsDraggingFromHandle(),
          });
