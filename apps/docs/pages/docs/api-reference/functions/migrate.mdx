...
An object mapping component names to custom migration functions. Used during the [DropZones to slots migration](#dropzones-to-slots) to handle complex zone migration scenarios where automatic migration isn't possible, such as migrating to [`slot`](/docs/api-reference/fields/slot) arrays.
