...
This field does not render any UI in the form section.

<Callout type="info">
  Core renders the transformed slot prop as a React component. Vue renders it
  with `<component :is>`, while Svelte places it by name with `<PuckSlot>`. See
  the [framework examples](/docs/integrating-puck/multi-column-layouts#nested-components).
</Callout>
