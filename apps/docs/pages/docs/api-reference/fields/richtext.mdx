...
import { Render } from "@/core/components/RichTextEditor/Render";
...
<ConfigPreview
  ...
  componentConfig={{
    ...
    render: ({ richtext }) => {
      return <Render content={richtext} />;
    },
  }}
/>
...
Configure the toolbar menu layout for the available [controls](#controls). Items are placed into groups.
...
Configure the toolbar menu layout for the available [controls](#controls) shown inline when enabling [`contentEditable`](#contenteditable). Items are placed into groups.
...
Controls are actions shown in the [`menu`](#menu) and [`inlineMenu`](#inlinemenu). You can define your own controls, or override the [default ones](#default-controls).
