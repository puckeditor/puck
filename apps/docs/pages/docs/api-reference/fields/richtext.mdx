  ...
  componentConfig={{
    fields: {
      richtext: {
        type: "richtext",
        options: {
          paragraph: { HTMLAttributes: { style: "margin-block: 8px;" } },
        },
      },
    },
    defaultProps: {
...
| [`renderInlineMenu()`](#renderinlinemenuprops) | `renderMenu: () => <RichTextMenu />` | Function   | -        |
...
```tsx {9-14, 18-20} copy showLineNumbers
import { RichTextMenu } from "@measured/puck";
