...
Get the label to show once the item is selected. Can return a string or ReactElement for rich content.
...
#### Using ReactElement for rich summaries

The `getItemSummary` function can return a ReactElement for more complex item summaries with styling and additional information.
...
<ConfigPreview
  label="External with ReactElement summary"
  componentConfig={{
    fields: {
      data: {
        type: "external",
