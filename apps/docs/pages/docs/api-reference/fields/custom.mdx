...
<Callout type="info">
  In `VueConfig` or `SvelteConfig`, `render` is a native component receiving the
  same field props. See the [framework
  examples](/docs/extending-puck/custom-fields#creating-a-custom-field).
</Callout>
