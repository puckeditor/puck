...
| [`uiStyles`](#uistyles)                                           | `uiStyles: false`                                  | `false \| { bundle?: "default" \| "no-external" }`                       | -            |
...
| [`syncStyles`](#syncstyles)       | `syncStyles: false`    | boolean | -      |
...
Set `syncStyles: false` to fully isolate the iframe from host styles and host `html` / `body` attributes while still keeping Puck's iframe interaction styles active.

### `uiStyles`

Configure runtime loading of the Puck editor UI styles.

```tsx {4} copy
export function Editor() {
  return (
    <Puck
      uiStyles={{ bundle: "no-external" }}
      // ...
    />
  );
}
...
| Param               | Example                 | Type                         | Status |
| ------------------- | ----------------------- | ---------------------------- | ------ |
| [`bundle`](#bundle) | `bundle: "no-external"` | `"default" \| "no-external"` | -      |

Pass `false` to disable runtime style injection entirely and load the CSS yourself.

##### `bundle`

Choose which built-in editor UI bundle to inject. Defaults to `"default"`.

- `"default"` loads the standard editor bundle, including the Inter font import.
- `"no-external"` omits the external font import so you can supply your own font.
