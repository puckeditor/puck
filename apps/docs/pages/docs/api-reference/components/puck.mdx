...
<Callout type="info">
  This page documents the core React component. Vue and Svelte share the config,
  data, and most props, but use native event or callback naming and have some
  advanced-API differences. See [Using with
  Vue](/docs/integrating-puck/using-with-vue#puck-props-and-events) or [Using
  with
  Svelte](/docs/integrating-puck/using-with-svelte#puck-props-and-callbacks).
</Callout>
