...
Show a drag handle in the component action bar. When [`behavior`](#behavior) is `"auto"`, drags started from the handle show a static line placeholder instead of animating sibling items.
