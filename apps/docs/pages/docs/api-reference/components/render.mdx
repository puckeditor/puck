...
<Callout type="info">
  Import `<Render>` from your framework package and provide its native config.
  The Vue and Svelte adapters currently mount `<Render>` client-side and do not
  emit published page content during SSR or SSG.
</Callout>
