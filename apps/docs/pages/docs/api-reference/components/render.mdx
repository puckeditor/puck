...
Transforms run on every render and may return components, unlike [`resolveData`](/docs/api-reference/configuration/component-config#resolvedatadata-params), which must return JSON and writes back to your [`Data`](/docs/api-reference/data-model/data) payload.
