...
<Callout type="warning">
  Vue and Svelte adapters can receive core plugins through the `plugins` prop,
  but plugin UI and field transforms are still Preact-based. Native Vue or
  Svelte plugin components are not yet supported.
</Callout>
