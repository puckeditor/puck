...
<Callout type="warning">
  Native Vue and Svelte overrides are not yet supported. Adapter `overrides`
  pass through to the Preact-based core API. To replace built-in field UIs with
  native components, use the adapter's `fieldTypes` prop instead.
</Callout>
