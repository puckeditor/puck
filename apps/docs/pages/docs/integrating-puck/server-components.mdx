...
```tsx copy filename="components/HeadingBlock.tsx" {1}
"use client";
...
```tsx copy filename="puck-types.ts"
import type { Config } from "@measured/puck";
...
```tsx copy filename="puck.config.client.tsx"
...
```tsx copy filename="puck.config.server.tsx"
import type { UserConfig } from "./puck-types.ts";
