...
<Callout type="warning">
  This guide applies to `@puckeditor/core`. The Vue and Svelte adapters mount
  `<Render>` client-side and currently emit an empty `div` on the server, so
  they do not yet provide SSR or SSG output for published pages. Their
  framework-agnostic utilities, such as `resolveAllData` and `migrate`, remain
