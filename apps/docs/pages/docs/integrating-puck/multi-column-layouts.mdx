...
Add the [`slot`](/docs/api-reference/fields/slot) field to your component to
create a zone that you can drop components into. Slot data is the same in every
framework, but each adapter exposes its native way to place the editable slot:
...
```ts {6-8} showLineNumbers copy filename="puck.config.ts"
...
<script setup lang="ts">
import type { VueSlot } from "@puckeditor/vue";
defineProps<{ leftColumn?: VueSlot; rightColumn?: VueSlot }>();
</script>

<template>
  <div class="columns">
    <component :is="leftColumn" />
    <component :is="rightColumn" />
  </div>
</template>
...
<div class="columns">
  <PuckSlot name="leftColumn" />
  <PuckSlot name="rightColumn" />
</div>
