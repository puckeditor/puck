...
## Opting out of host style sync

By default, Puck mirrors host styles into the preview iframe so your components render with the same app CSS.

Disable that sync with `iframe.syncHostStyles` when you need the preview iframe to be fully isolated:
...
This keeps Puck's iframe interaction styles active, but it stops mirroring host stylesheets and host `html` / `body` attributes.
