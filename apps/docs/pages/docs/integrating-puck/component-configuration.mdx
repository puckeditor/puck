...
Components are defined via the `components` object in
[Config](/docs/api-reference/configuration/config). Every definition must
provide a [`render`](/docs/api-reference/configuration/component-config#renderprops).
Core uses a React render function; the Vue and Svelte adapters use a native
component:
...
```ts showLineNumbers copy {1,6} filename="puck.config.ts"
...
If you're using TypeScript, we recommend strictly typing your config with the
type exported by your framework package:
...
const config: VueConfig<Components> = {
  components: {
    HeadingBlock: { render: HeadingBlock },
  },
};
```
...
const config: SvelteConfig<Components> = {
  components: {
    HeadingBlock: { render: HeadingBlock },
  },
};
...
[Fields](/docs/api-reference/fields) allow users to provide input to components.
The value of each field is passed as a prop to the render function or native
framework component.
...
```ts showLineNumbers copy {5-7} /title/2,3 filename="puck.config.ts"
const config = {
  components: {
    HeadingBlock: {
      fields: {
...
It's best to define the props for the component if using TypeScript. This
enables strict type checking for your fields. Use `VueConfig` or `SvelteConfig`
in place of core's `Config` when using those adapters.
