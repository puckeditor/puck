...
To use rich text, add the `richtext` field to your component config. Rich text
is node-valued: React receives the node as a prop, Vue receives an outlet
component, and Svelte renders it by field name with `<PuckText>`.

<FrameworkTabs>
  <FrameworkTab>

```tsx copy showLineNumbers {6,9}
const config = {
  components: {
    Example: {
      fields: {
...
```ts copy showLineNumbers {6,9} filename="puck.config.ts"
...
<Callout type="warning">
  Native Vue and Svelte menu components are not yet supported. `renderMenu`
  passes through to the Preact-based core API, so the examples in this section
  apply to core or to advanced adapter configs authored with Preact components.
</Callout>
