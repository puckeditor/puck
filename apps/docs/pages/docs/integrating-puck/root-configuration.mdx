...
Use the [`root` parameter](/docs/api-reference/configuration/config#root) to
specify a renderer. The root must render its `children` outlet for the page's
top-level components to appear:
...
```ts showLineNumbers copy {5-7} filename="puck.config.ts"
import Root from "./Root.vue";

const config = {
  components: { HeadingBlock: { render: HeadingBlock } },
  root: {
    render: Root,
  },
};
...
```vue copy filename="Root.vue"
<script setup lang="ts">
import type { VueSlot } from "@puckeditor/vue";
defineProps<{ children?: VueSlot }>();
</script>

<template>
  <main><component :is="children" /></main>
</template>
```
...
```svelte copy filename="Root.svelte"
<script lang="ts">
  import { PuckChildren } from "@puckeditor/svelte";
</script>

<main><PuckChildren /></main>
```
...
Generic types can be passed to `Config`, `VueConfig`, or `SvelteConfig` to
strictly type your root configuration:
