...
You can avoid FOUC by bundling the `puck.css` exported by your framework
package:
...
Puck includes references to external font packages. Use your framework
package's `no-external.css` bundle to exclude them:
...
```css copy
@import "@puckeditor/vue/no-external.css";
```
...
```css copy
@import "@puckeditor/svelte/no-external.css";
```
